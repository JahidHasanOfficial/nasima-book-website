...
<body>
  ...
  if (isset($_GET['edit_id'])) {
    $eq = mysqli_query($con, "SELECT * FROM project_category WHERE id='" . $_GET['edit_id'] . "' ");
  ...
  if (isset($_POST['btnsave'])) {
    ...
    if (!isset($errMSG)) {
      if ($id == 0) {
        $stmt = $DB_con->prepare('INSERT INTO project_category (user_id,cat_name,cat_details,link,cat_photo) 
  ...
  ?>
        ...
        $stmt = $DB_con->prepare('UPDATE project_category  SET  cat_name =:CatName, cat_details=:Details,cat_photo=:upic,link=:Link WHERE id=:uid');
  ...
  <main id="main" class="main">
    ...
    <div class="pagetitle">
      <h1>Add / Edit Project Category | <span> <a href="project-category"> <i class="bi bi-table"></i> </a> </span> </h1>
      <hr>
    </div>
