...
<body>
  ...
  if (isset($_POST['btn_save_updates'])) {
    ...
    function createSlug($title)
{
    // Step 1: Remove all special characters except Bangla, English letters, numbers, space and hyphen
    $slug = preg_replace('/[^\p{L}\p{N}\s-]+/u', '', $title);

    // Step 2: Replace multiple spaces or hyphens with a single space
    $slug = preg_replace('/[\s-]+/', ' ', $slug);

    // Step 3: Replace spaces with hyphen
    $slug = preg_replace('/\s+/', '-', $slug);

    // Step 4: Trim hyphens from both ends
    $slug = trim($slug, '-');

    // Step 5: Make lowercase (for English letters only)
    ...
    if (!isset($errMSG)) {
      $stmt = $DB_con->prepare('UPDATE blog_section  SET  category_id=:CategoryId, title =:Title, slug=:Slug, details=:Details, userPic=:upic, canonical_link = :CanonicalLink, keyword = :Keyword, description = :Description, author = :Author WHERE id=:uid');
              ...
              <form method="post" enctype="multipart/form-data" class="form-horizontal">
                ...
                <table class="table table-responsive">
                 ...
                 <tr class="d-none">
                    ...
                    <td>
                      <select name="category_id" class="form-select"> 
                        ...
                        while ($cat_row = mysqli_fetch_array($cat_query)) {
                          $selected = ($cat_row['id'] == $category_id) ? 'selected' : '';
                  ...
                  <tr class="d-none"><tr class="d-none">
                  ...
                  <tr class="d-none">
                    ...
                    <td>
                      <input type="text" class="form-control" name="canonical_link" placeholder="SEO Canonical Link" value="<?= $canonical_link ?>" ></input>
                    </td>
                  </tr>
                  ...
                  <tr class="d-none">
                    ...
                    <td>
                      <textarea class="form-control" name="keyword" placeholder="SEO Keyword" id="" cols="30" rows="4" ><?= $keyword ?></textarea>
                    </td>
                  </tr>
                 ...
                 <tr class="d-none">
                    ...
                    <td>
                      <textarea class="form-control" name="description" placeholder="SEO Description" id="" cols="30" rows="4" ><?= $description ?></textarea>
                    </td>
                  </tr>
